...
public class TridentProcessService implements Callable<Map<String, Set<ActualSupplierData>>> {
    ...
    /*public static void main(String args[]) throws InterruptedException, JsonProcessingException {
        ...
        TridentProcessService process = new TridentProcessService();
        ...
        *//*cascadeDataForTrident.put("BD Micro-Fine Ultra hypodermic insulin needles for pre-filled / reusable pen injectors screw on 4mm/32gauge Pk: 100",
        ...
        Map<String,Set<ActualSupplierData>> processedTridentData = process.processTrident(cascadeResultsMap);
    ...
    public Map<String,Set<ActualSupplierData>> call() throws InterruptedException {
        ...
        for(Map.Entry<String,Set<ActualSupplierData>> entry: cascadeDataForTrident.entrySet()){
            ...
            for(ActualSupplierData tridentData : tridentDataSet){
                if("Trident Pharmaceuticals".equalsIgnoreCase(tridentData.getSupplier())
                        && !StringUtils.isEmpty(tridentData.getSupplier())){
                    ...
                    Thread.sleep(3000);*/
                        ...
                        tridentData.setSupplierPrice(!StringUtils.isEmpty(priceFromWebsite) ?Double.valueOf(priceFromWebsite.replace("£","")):null);
                        tridentData.setDefinitePrice(!StringUtils.isEmpty(priceFromWebsite) ?Double.valueOf(priceFromWebsite.replace("£","")):null);
                        tridentData.setSupplierStatus(!StringUtils.isEmpty(availablityFromWebsite)?stockAvailability(availablityFromWebsite):null);
                        tridentData.setDefiniteStatus(!StringUtils.isEmpty(availablityFromWebsite)?stockAvailability(availablityFromWebsite):null);
                        //tridentData.setSupplier("Trident Pharmaceuticals");
